...
for (i in 2:11) {
    ...
    ## <X> loop is an artifact of the general procedure
    for (j in 1:1) {
        ...
        out.filename <- paste(tmp.panel,".","Group_",groups[j],".gbmCaretFit_NeLq_REPCV.Rdata",sep="")
        
        ##------------------------------------------------------------------
        ## <X> define the dependent variable and the last-quoted benchmark
        ##------------------------------------------------------------------
        ...
        ##------------------------------------------------------------------
        ## <X> added the target back, so I can drop
        ...
        tmpClass  <- as.factor(ifelse(tmpClass == 1, "NOTLQ","LQ"))

...
for (i in 2:2) {
    ...
    for (j in 7:7) {
        ...
        ftmpDescr       <- tmpDescr[, -nzv]
        ftestDescr      <- testDescr[, -nzv]
        
        ##------------------------------------------------------------------
        ## remove highly-correlated predictors
        ##------------------------------------------------------------------
        descrCor        <- cor(ftmpDescr)
        hcv             <- findCorrelation(descrCor, cutoff = 0.75)
        hftmpDescr      <- ftmpDescr[, -hcv]
        hftestDescr     <- ftestDescr[, -hcv]

        
        ## sync testDescr names with tmpDescr names
        misMatchCols <- which( !(colnames(hftmpDescr) %in% colnames(hftestDescr)) )
        if (length(misMatchCols) > 0) {
            hftmpDescr  <- hftmpDescr[ , -misMatchCols]
        }
        misMatchCols <- which( !(colnames(hftestDescr) %in% colnames(hftmpDescr)) )
        if (length(misMatchCols) > 0) {
            hftestDescr  <- hftestDescr[ , -misMatchCols]
        }
        ...
        #
        #set.seed(88888888)
        #holdSmp     <- sample.int(nrow(hftmpDescr), round(0.20*nrow(hftmpDescr))) ## 10% hold-out
        #
        ...
        svmGrid <- expand.grid(.sigma=svm.s, .C=svm.c)
        ...
        ##system.time({
        ## perform a fit
        tmp.fit <- try(train(   x=hftmpDescr,
                                y=tmpClass,
                                method="svmRadial",
        ...
        if (class(tmp.fit)[1] == "try-error") {
            ...
            save(tmp.fit, tmpClass, hftmpDescr, tmp.data, hftestDescr, test.data, file=out.filename)
        }
    }
}

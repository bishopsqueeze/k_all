...
## Set the working directory
##------------------------------------------------------------------
setwd("/Users/alexstephens/Development/kaggle/allstate/data/gbm_scored_X009")
...
for (i in 2:11) {
    ...
    for (j in 1:7) {
        ...
        tmp.loadfile <- paste(tmp.sp,".Group_",tmp.ch,".svmCaretFit_AllSample_REPCV.Rdata",sep="")
        load(tmp.loadfile)
...
for (i in 2:11) {
    ...
    tmp.panel <- paste("/Users/alexstephens/Development/kaggle/allstate/data/panels/X005_allstatePanelData_Train.",tmp.sp,".Rdata",sep="")
    load(tmp.panel)
...
save(xcheck.list, ch.combos, xcheck.out, res.holdout, optimal.plans,
            file="/Users/alexstephens/Development/kaggle/allstate/data/S009_CaretSvmXcheck.Rdata")

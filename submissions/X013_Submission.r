...
## Set the working directory
##------------------------------------------------------------------
setwd("/Users/alexstephens/Development/kaggle/allstate/data/gbm_scored_X013")
...
## save in submission format
#gbm_bonly.fin <- gbm_bonly.sub[ ,c("customer_ID","plan")]
...
## save in submission format
#gbm_eonly.fin <- gbm_eonly.sub[ ,c("customer_ID","plan")]
...
## save in submission format
#gbm_fonly.fin <- gbm_fonly.sub[ ,c("customer_ID","plan")]
...
## index of changes vs. last quoited benchmark
...
##******************************************************************
write.csv(lastquoted.fin,   file="X011_lastquoted.csv", row.names=FALSE)
write.csv(gbm_gonly.fin,    file="X011_gbm_gonly.csv", row.names=FALSE)
#write.csv(gbm_gc,           file="X011_gbm_gc.csv", row.names=FALSE)
